...
struct MyApp : DistributedAppWithState<CommonState> {
  ...
  Parameter grainPositionMin{"/grainPositionMin", "", 0.0, 0.0, 0.1};
  Parameter grainPositionMax{"/grainPositionMax", "", 0.0, 0.0, 0.1};
  ...
  if (isPrimary()) {
    state().MAX_PARTICLES = 50000;
    state().slope = fadeSlope;
    state().freeze = false;
    state().time = 0;
    state().delay = 0;
    state().pointSize = (grainRateMax + grainRateMin) / 2;
  }
  }
  ...
  void onCreate() override {
    // compile shaders
    if (isPrimary()){
    state().pointShader.compile(slurp("../point-vertex.glsl"),
    ...
    state().mesh.primitive(Mesh::POINTS);
    if (isPrimary()) {
    for (int _ = 0; _ < state().MAX_PARTICLES; _++) {
      state().mesh.vertex(randomVec3f(1));
      state().mesh.color(randomColor());

      // float m = rnd::uniform(3.0, 0.5);
      float m = 3 + rnd::normal() / 2;
      if (m < 0.5) m = 0.5;
      state().mass.push_back(m);

      // using a simplified volume/size relationship
      state().mesh.texCoord(pow(m, 1.0f / 3), 0);  // s, t

      // separate state arrays
      state().velocity.push_back(randomVec3f(0.1));
      state().force.push_back(randomVec3f(1));
    }
    }
  ...
  void onAnimate(double dt) override {
    ...
    for (int i = 0; i < state().velocity.size(); i++) {
      state().X0 = state().mesh.vertices()[i]; //resting 
      state().X = (state().X0 / state().X0.mag()) * sphereRadius; //distance
      state().F = -1 * hookeConstant * (state().X0 - state().X); //hooke equation
      state().force[i] += state().mass[i] * state().F; //implement 
      }
    }
    ...
    if (isPrimary()){
    for (int i = 0; i < state().velocity.size(); i++) {
      state().force[i] += - state().velocity[i] * dragFactor;
    }
    }

    vector<Vec3f> &position(state().mesh.vertices());
    ...
    for (int i = 0; i < state().velocity.size(); i++) {
      ...
      state().velocity[i] += (state().force[i] / state().mass[i] * (impulseRate * 0.25)) * (value + (parameter[0] * 10));
      position[i] += (state().velocity[i] * (impulseRate * 0.125)) * value;
    }
    }


    for (auto &a : state().force) a.set(0);
  ...
  void onSound(AudioIOData& io) override {
    ...
    tmr.period(1 / impulseRate);
      if(tmr()){
        env.attack(state().slope);
        env.decay(1-state().slope);
        env.amp(1.0);
        env.reset();

        state().slope +=0.01;
        if(state().slope > 1) state().slope = fadeSlope;
      }
    ...
    if (isPrimary()){
    for (int i = 0; i < state().velocity.size(); i++) {
      state().force[i] += Vec3f(rnd::uniform(10,1) * parameter[0] * value * 5, rnd::uniform(10,1) * parameter[1] * value * 5, rnd::uniform(10,1)* parameter[2] * value * 5);
      state().force[i] -= Vec3f(rnd::uniform(10,1) * parameter[0] * value * 5, rnd::uniform(10,1) * parameter[1] * value * 5, rnd::uniform(10,1)* parameter[2] * value * 5);
